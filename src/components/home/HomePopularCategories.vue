  ...
  <section class="mb-12 font-sans">
    ...
    <div v-if="isLoading" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <div
        v-for="i in 4"
        :key="i"
        class="relative overflow-hidden rounded-lg h-40 md:h-48 bg-slate-200 animate-pulse"
      >
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      >
        ...
        <div
          class="absolute inset-0 bg-linear-to-t from-slate-900/90 via-slate-900/30 to-transparent"
        ></div>
        ...
        <div class="absolute inset-0 p-5 flex flex-col justify-end text-white">
          ...
          <!-- 🌟 កូដដែលបាត់៖ បន្ថែមការបង្ហាញ Sub-categories (children) នៅទីនេះជាទម្រង់ Tags តូចៗ -->
          <ul v-if="cat.children && cat.children.length > 0" class="flex flex-wrap gap-2 mb-4">
            <li
          ...
          <div class="flex items-center justify-between mt-auto">
            ...
            <span
              class="text-sm font-bold text-blue-400 flex items-center gap-1 group-hover:gap-2 group-hover:text-white transition-all"
            >
              Shop Now
