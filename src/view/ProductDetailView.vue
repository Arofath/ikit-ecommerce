  ...
  <div class="container mx-auto px-4 lg:px-8 max-w-7xl py-6 font-sans">
    ...
    <div v-else-if="productStore.hasProduct">
      ...
      <RelatedProduct
        ...
        :isLoading="productStore.isRelatedLoading"
      />
    </div>
    <!-- នេះទើបជាកន្ទុយបិទពិតប្រាកដរបស់ v-else-if -->
  </div>
</template>
...
onMounted(() => {
  ...
  if (slug) {
    ...
    productStore.fetchRelatedProducts(slug)
  }
})
...
watch(
  ...
  (newSlug) => {
    if (newSlug) {
      ...
      productStore.fetchRelatedProducts(newSlug)
